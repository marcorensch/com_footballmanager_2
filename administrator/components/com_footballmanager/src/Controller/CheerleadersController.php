 ...
 * @since  2.0.0
 */
class CheerleadersController extends AdminController
{
    /**
     * Constructor.
     *
     ...
     * @since   2.0.0
     */
    public function getModel($name = 'cheerleader', $prefix = 'Administrator', $config = ['ignore_request' => true])
    {
	...
	public function export()
	{
		...
		$model = $this->getModel('cheerleaders');
		$data  = $model->exportItems($ids);

		ExportHelper::export($data, 'cheerleaders');
	}
...
//			move_uploaded_file($tmpFilePath, $destinationPath);
		...
		$app->enqueueMessage('Importing Cheerleaders is not supported yet', 'warning');
		$this->setRedirect(Route::_('index.php?option=com_footballmanager&view=cheerleaders', false));
	}
