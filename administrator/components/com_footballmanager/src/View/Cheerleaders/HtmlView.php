...
namespace NXD\Component\Footballmanager\Administrator\View\Cheerleaders;
...
class HtmlView extends BaseHtmlView
{
	...
	{
        try {
            /* @var \NXD\Component\Footballmanager\Administrator\Model\CheerleadersModel $model */
            $model = $this->getModel();
	...
	protected function addToolbar(): void
	{
        ...
        ToolbarHelper::title(Text::_('COM_FOOTBALLMANAGER_CHEERLEADERS'), 'fas fa-smile');
		...
		{
			$toolbar->addNew('cheerleader.add');
		}
		if ($canDo->get('core.edit.state'))
		{
			...
			$childBar->publish('cheerleaders.publish')->listCheck(true);
			$childBar->unpublish('cheerleaders.unpublish')->listCheck(true);
			$childBar->archive('cheerleaders.archive')->listCheck(true);


			if ($user->authorise('core.admin'))
			{
				$childBar->checkin('cheerleaders.checkin')->listCheck(true);
			}

			if ($this->state->get('filter.published') != -2)
			{
				$childBar->trash('cheerleaders.trash')->listCheck(true);
			}
		}

		if ($this->state->get('filter.published') == -2 && $canDo->get('core.delete'))
		{
			$toolbar->delete('cheerleaders.delete')
...
//			ToolbarHelper::custom('cheerleaders.export', 'download', '', 'COM_FOOTBALLMANAGER_EXPORT', true);
		}
