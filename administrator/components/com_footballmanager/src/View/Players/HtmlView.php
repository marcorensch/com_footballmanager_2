...
namespace NXD\Component\Footballmanager\Administrator\View\Players;
...
class HtmlView extends BaseHtmlView
{
    ...
    {
        try {
            /* @var \NXD\Component\Footballmanager\Administrator\Model\PlayersModel $model */
            $model = $this->getModel();
    ...
    protected function addToolbar(): void
    {
        ...
        ToolbarHelper::title(Text::_('COM_FOOTBALLMANAGER_PLAYERS'), 'fas fa-users');
        ...
        if ($canDo->get('core.edit.state')) {
            ...
            $childBar->publish('players.publish')->listCheck(true);
            $childBar->unpublish('players.unpublish')->listCheck(true);
            $childBar->archive('players.archive')->listCheck(true);


            if ($user->authorise('core.admin')) {
                $childBar->checkin('players.checkin')->listCheck(true);
            }

            if ($this->state->get('filter.published') != -2) {
                $childBar->trash('players.trash')->listCheck(true);
            }
        }

        if ($this->state->get('filter.published') == -2 && $canDo->get('core.delete')) {
            $toolbar->delete('players.delete')
...
//			ToolbarHelper::custom('players.export', 'download', '', 'COM_FOOTBALLMANAGER_EXPORT', true);
        }
