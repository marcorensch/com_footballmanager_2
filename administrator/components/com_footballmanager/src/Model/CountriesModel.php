...
class CountriesModel extends ListModel
{
    ...
    protected function getListQuery()
    {
        ...
        $query->select(
            $db->quoteName(
				[
					'a.id', 'a.title', 'a.alias', 'a.created_at', 'a.created_by',
					'a.published', 'a.ordering', 'a.state',
				]
            )
        );
        $query->from($db->quoteName('#__footballmanager_countries','a'));
	...
	public function exportItems($ids)
	{
		...
		$query = $db->getQuery(true);
		$query->select('*');
		$query->from($db->quoteName('#__footballmanager_countries'));
