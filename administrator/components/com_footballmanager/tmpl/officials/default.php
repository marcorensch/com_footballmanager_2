...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    downloadBtn.addEventListener("click", ()=>{
        taskField[0].value = 'officials.download';
    });
});
JS
);
...
{
	$saveOrderingUrl = 'index.php?option=com_footballmanager&task=officials.saveOrderAjax&tmpl=component&' . Session::getFormToken() . '=1';
...
?>
    ...
    <form action="<?php echo Route::_('index.php?option=com_footballmanager&view=officials'); ?>" method="post"
					...
					<?php else : ?>
                        <table class="table playerList" id="playersList">
							...
							foreach ($this->items as $i => $item) :
								...
								?>
                                <tr class="row<?php echo $i % 2; ?>" data-draggable-group="refereeItems">
                                    ...
                                    <td class="text-center">
										<?php
										echo HTMLHelper::_('jgrid.published', $item->published, $i, 'referees.', $canChange, 'cb');
										?>
                                    </td>
                                    <th scope="row" class="has-context">
                                        <a class="hasTooltip"
                                           href="<?php echo Route::_('index.php?option=com_footballmanager&task=official.edit&id=' . (int) $item->id); ?>"
